...
for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes('case "rekomendasi_add": return `')) {
        start = i;
    }
    if (start !== -1 && lines[i].includes('// ── 6. TUNDA SALUR ADD')) {
        end = i;
        break;
    }
}
...
if (start !== -1 && end !== -1) {
    const newContent = `    case "rekomendasi_add": return \`
      ...
      <div class="print-page" style="\${base};page-break-before:always;">
        ...
        <p style="font-family:Arial;font-size:12pt;text-align:justify;margin-bottom:24px;text-indent:40px;line-height:1.5;">
...
`;
    ...
    console.log('Rekomendasi ADD updated');
} else {
